...
################################################################################
#Deleting the course by providing specific id
def delete_course(course_id):
    course_to_delete = list_courses()
    if course_id in course_to_delete:
        try:
            course = service.courses().get(id=course_id).execute()
            course['courseState'] = 'ARCHIVED'
            course = service.courses().delete(id=course_id).execute()
            print('course deleted')
            logging.info('Deleting the specific course by providing specific id')
        except errors.HttpError as error:
